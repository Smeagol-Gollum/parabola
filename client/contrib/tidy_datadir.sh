...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Parabola database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Parabola datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Parabola datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Parabola 0.7 datadir."
    ;;
  3)
    echo "Detected Parabola pre-0.8 datadir."
    ;;
  4)
    echo "Detected Parabola 0.8 datadir."
    ;;
esac
