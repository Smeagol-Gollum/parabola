...
{
    QString ret = QString("parabola:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non X^2 unit input in lnReqAmount, we generate the URI with X^2 as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(ParabolaUnits::format(ParabolaUnits::X^2, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
